...
__global__ void CalculateDistance(const uint32_t *XXT, const uint32_t *norms,
    ...
    if (row < m && col < m && row <= col) {
        ...
        // Prevenir bajo flujo (underflow) por si acaso, aunque matemáticamente sum_norms >= 2 * xxt
        if (sum_norms > 2 * xxt) {
...
__global__ void XXT_WMMA_Shared(const uint8_t *X, uint32_t *C, int m, int n) {
    ...
    // El acumulador en memoria compartida sigue siendo int32_t debido a la salida de store_matrix_sync
    ...
    wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, int> c_frag; // Obligatorio 'int' por API WMMA
    ...
    for (int idx = threadIdx.x; idx < TILE_M * TILE_N; idx += blockDim.x) {
        ...
        if (global_row < m && global_col < m && global_row <= global_col) {
            ...
            C[packed_idx] = (uint32_t)c_tile[local_row][local_col]; // Cast explícito a unsigned int
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    uint32_t *d_XXT = NULL;        // Cambiado a uint32_t
    ...
    if (m <= 64 && n <= 1024) {
        ...
        if (!h_distances) {
            ...
            print_packed_matrix_half(h_distances, m); 
        }
        free(h_norms);
        if (h_XXT) free(h_XXT);
