...
job_e3=$(sbatch --parsable --output=e3_1.out --error=e3_1.err --open-mode=truncate lanzar.sh "./ej2_1" 64 1)
...
job_e4=$(sbatch --parsable --output=e4_1.out --error=e4_1.err --open-mode=truncate lanzar.sh "./ej2_2" 64 1)
...
job_e2_1=$(sbatch --parsable \
  ...
  lanzar.sh "./ej2" --nsys shared_memory_256M_alt)

job_e3_2=$(sbatch --parsable \
  --output=e3_2.out --error=e3_2.err \
  lanzar.sh "./ej2_1" --nsys shared_memory_256M)

job_e4_2=$(sbatch --parsable \
  --output=e4_2.out --error=e4_2.err \
  lanzar.sh "./ej2_2" --nsys warp_shuffle_256M)

printf "Jobs enviados: e1=%s, e1_2=%s, e1_3=%s, e2=%s, e2_1=%s, e3=%s, e3_2=%s, e4=%s, e4_2=%s\n" "$job_e1" "$job_e1_2" "$job_e1_3" "$job_e2" "$job_e2_1" "$job_e3" "$job_e3_2" "$job_e4" "$job_e4_2"

for jid in "$job_e1" "$job_e1_2" "$job_e1_3" "$job_e2" "$job_e2_1" "$job_e3" "$job_e3_2" "$job_e4" "$job_e4_2"; do
